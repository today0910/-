    ...
    , ui(new Ui::MainWindow)
{
    ...
    //pause
    connect(gam->pausebtn,&MyPushButton::clicked,[=](){
        gam->m_gameTimer->stop();  // 停止计时器
        gam->pausebtn->zoom1();
        gam->pausebtn->zoom2();
        gam->pause->show();
        shade->show();
        gam->pause->raise(); // 提升pause窗口到最上层
    });
    connect(gam->pause,&Pause::menus,[=](){
        gam->reset();
        gam->hide();
        gam->pause->hide();
        shade->hide();
    ...
    connect(gam->pause,&Pause::continue_,[=](){
        gam->m_gameTimer->start(1000);  // 重新启动计时器
        gam->pause->hide();
        shade->hide();
    });
