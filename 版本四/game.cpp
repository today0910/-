    ...
    m_elapsedSeconds(0)
{
    ...
    connect(m_gameTimer, &QTimer::timeout, this, &game::checkTime);
    m_gameTimer->start(1000);  // 每秒检查一次
    //设置遮罩
    ...
    pause=new Pause;
    //设置pause按钮的图片和大小
    pausebtn=new MyPushButton(":/image/pause.png",0.5,0.5);
    pausebtn->setParent(this);
    //设置pause按钮的位置
    int x = this->width() - pausebtn->width() ;
    int y = this->height() - pausebtn->height() ;
    pausebtn->move(x, y);
...
bool game::getMaxAndMin(int nowx, int nowy,int fow)
{
    ...
    {
        move_ = 0;
        if(rednumber+bluenumber==8){
            emit this->winend1();
        }
        else if(rednumber+bluenumber==7){
            emit this->winend2();
        }
        else if(rednumber+bluenumber==6){
            emit this->winend3();
        }
        else if(rednumber+bluenumber==5){
            emit this->winend4();
        }
        else if(rednumber+bluenumber==4){
            emit this->winend5();
        }
        else{
            emit this->loseend();
    ...
    //吃钻石
    if(nowx>=390&&nowx<=430&&nowy>=510&&nowy<=550&&fow==0){
        if(redget[0]==false){
    ...
    if(nowx>=520&&nowx<=560&&nowy>=350&&nowy<=390&&fow==0){
        if(redget[1]==false){
    ...
    if(nowx>=200&&nowx<=240&&nowy>=30&&nowy<=70&&fow==0){
        if(redget[2]==false){
    ...
    if(nowx>=710&&nowx<=750&&nowy>=150&&nowy<=170&&fow==0){
        if(redget[3]==false){
    ...
    if(nowx>=560&&nowx<=600&&nowy>=510&&nowy<=540&&fow==1){
        if(blueget[0]==false){
    ...
    if(nowx>=470&&nowx<=510&&nowy>=240&&nowy<=280&&fow==1){
        if(blueget[1]==false){
    ...
    if(nowx>=55&&nowx<=95&&nowy>=90&&nowy<=130&&fow==1){
        if(blueget[2]==false){
    ...
    if(nowx>=420&&nowx<=460&&nowy>=30&&nowy<=70&&fow==1){
        if(blueget[3]==false){
...
void game::checkTime()
{
    ...
    // 检查是否超过3分钟（180秒）
    if (m_elapsedSeconds >= 180) {
        m_gameTimer->stop();  // 停止计时器
        move_ = 0;            // 停止角色移动
        emit this->loseend(); // 触发游戏失败信号
    }
}
